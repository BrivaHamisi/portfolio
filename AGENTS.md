...
- Analytics is Vercel Analytics via `@vercel/analytics/vue`'s `<Analytics />` component in `App.vue` — don't replace it with a different analytics tool without being asked.
...
- `photographyProjects` in `src/data/work.js` holds one `demo: true` placeholder entry (`public/images/photography/demo-*.webp`, freely-licensed stock photos, added 2026-09-11 at the user's explicit request so the tab isn't empty) — it is not Briva's own photography. `demo-5` is a retained `.jpg` (webp didn't beat it on bytes at comparable quality). Do not populate it with unrelated existing images from other categories (testimonial headshots, UI mockups) either — that would misrepresent real people's photos or unrelated work as his. `PhotographyView.vue` shows a visible "Demo placeholder" badge driven by the `demo` flag; when real photography is supplied, replace the whole entry and delete the demo images rather than editing around them.
...
- The accessible modal used to live inline inside `latestWork.vue`; it's now `src/components/ProjectModal.vue`. If you're looking for the focus-trap/Escape/scroll-lock logic and it's not where you remember, that's why. The Tab trap is a document-level handler (registered while `show` is true) that also catches Tab when focus lands on the modal overlay/background — don't reintroduce a panel-scoped `@keydown.tab` trap, that's the pattern it replaced.
- Raster stills ship as **webp only**: the `.jpg`/`.png` source masters under `public/images/` were deleted in the 2026-09-13 asset-diet pass (with the user's permission), since `public/` is copied wholesale into `dist/`. Remaining `.jpg`/`.png` are exceptions with a reason (`brand/BH_Monogram.png` logo, `brand/og-image.jpg` social meta, `photography/demo-5.jpg`, testimonial avatars `Mamo_M.jpg`/`moses_ngure.jpg`). Serve new imagery as webp; if webp doesn't beat the jpg on bytes at comparable quality, keep the jpg and say why in the code comment. Also add any image shown at natural size (masonry items, lightbox large view) to the `workImageSizes` map in `src/data/work.js` so the browser reserves its ratio — the map drives `:width`/`:height` in `MasonryGrid.vue` and `ImageLightbox.vue`.
